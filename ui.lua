...
local fonts = {}; do
	...
	-- Fallback for Tahoma if GitHub URL is broken
	local TahomaFont
	pcall(function()
		TahomaFont = game:HttpGet("https://github.com/ravegirls/meow/raw/refs/heads/main/tahoma-bold.ttf")
	end)
	
	-- If that fails, try alternative source
	if not TahomaFont then
		pcall(function()
			TahomaFont = game:HttpGet("https://raw.githubusercontent.com/ravegirls/meow/main/tahoma-bold.ttf")
		end)
	end
	
	-- Final fallback: use empty font data to prevent errors
	TahomaFont = TahomaFont or ""
...
function library:toggle(options) 
    ...
    function cfg.set(bool)       
        ...
        -- Ensure bool is actually a boolean, not a table
